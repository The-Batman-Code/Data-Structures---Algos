...
def deleteDeepestNode(nodeRoot, dNode):
    if not nodeRoot:
        return None
    else:
        new_queue = Queue()
        new_queue.enqueue(nodeRoot)
        while not (new_queue.isEmpty()):
            root = new_queue.dequeue()
...
# print(deleteDeepestNode(newBT, getDeepestNode(newBT)))
# levelOrderTraversal(newBT)


def deleteNodeBT(nodeRoot, node):
    if not nodeRoot:
        ...
        while not (new_queue.isEmpty()):
            ...
            if root.value.data == node:
                dNode = getDeepestNode(nodeRoot)
                root.value.data = dNode.data
                deleteDeepestNode(nodeRoot, dNode)
...
def deleteBT(nodeRoot):
    nodeRoot.data = None
    nodeRoot.leftChild = None
    nodeRoot.rightChild = None
    return "The BT has been scccessfully deleted"
